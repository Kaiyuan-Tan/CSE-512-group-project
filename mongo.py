...
def delete():
    try:
        ...
        return jsonify({"message": f"User deleted successfully. See you again"}), 201
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
...
def search_history_update():
    try:
        ...
        return jsonify({"message": "User search history updated"}), 201
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
...
def read_books_update():
    try:
        ...
        resp = atlas_client.update(collection_name=COLLECTION_NAME, user_id={"email": email},new_value=update)
        return jsonify({"message": "User read books updated"}), 201
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
...
def search():
    try:
        ...
        # return 
        return jsonify({"message": "Requetst success", "data": pretty_response(response)}), 200
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
...
# Rate a book
@app.route("/elasticsearch/review", methods=["POST"])
def rating():
    try:
        data = request.json
        rating = data["rating"]
        ...
        if rating is None or not (0.0 <= rating <= 5.0):
            return jsonify({"success": False, "message": "Invalid rating value. Please provide a rating between 0 and 5."}), 400

        book = client.get(index=INDEX_NAME, id=book_id)["_source"]
        current_count = book["count"]
        current_rating= book["rating"]
        total_score = current_count * current_rating
        new_count = current_count + 1
        new_rating = (total_score + rating) / new_count
        ...
        response = client.update(index=INDEX_NAME, id=book_id, body = query)
        # print(response)
...
# Sorted by rating
@app.route("/elasticsearch/rating", methods=["GET"])
def rank_by_rating():
    try:
        order = request.args.get("order")
        if order!="asc" and order!="desc":
